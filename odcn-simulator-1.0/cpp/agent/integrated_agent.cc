...
void BurstManager::recv( Packet *pkt, Handler *h ) {
    ...
    if ( currburstsize_  >= maxburstsize__ )
        sendBurst();
    // Debug::debug( "BurstManager: added the data packet successfully" );
}
...
void IPKTAgent::recv( Packet *pkt, Handler *h ) {
    ...
    if ( (ch->ptype() != PT_IPKT)            //GMG -- changed this to
        ...
        if ( hdrip->prio_== 2 )   {
            ...
            sendBurst( hdr->C_burst_id_, (double)hdr->start_time_at_podswitch_, (double)hdr->start_time_at_coreswitch_, hdr->corenode_address_, hdr->wavelength_);
        }
...
void IPKTAgent::deBurst(Packet *pkt) {
    ...
    while ( npkts--) {
        ...
        Packet::free(*p); //drop data packet
        p++; //next data packet
    }
    Packet::free(pkt); //drop data burst
}
...
void IPKTAgent::sendBurst( u_long bid, double starttime1, double starttime2, int corenode, int channel ) {
	...
	int npkts  = p->getNPkts();
    u_long burstsize = p->getBurstSize();
	int sourcenode = p->getSrcNode();
	int destinationnode = p->getDestNode();
	double birthtime = p->getBirthTime();
    double offsettime = p->getOffsetTime();
	int pcntguard = p->getPcntGuard();
    ...
    // memory space allocation for the payload
    pkt_burst->allocdata(npkts*sizeof(Packet*));
    // move the payload to data burst
    memmove(pkt_burst->accessdata(),pBurst,npkts*sizeof(Packet*));
    ...
    hdr_IPKT_burst->prio_   = priority; //class of traffic
    hdr_IPKT_burst->delta_ = delta;
    hdr_IPKT_burst->pcntguard_ = pcntguard;
...
void IPKTAgent::sendControlPacket( Packet** pBurst, u_long bid, int npkts, u_long burstsize, int sourcenode, int destinationnode, double offsettime, int pcntguard, double delta, int priority ){
...
// command method - changed by Salek Riadi
