...
class IPKTAgent : public Agent
{
    public:
        ...
        void sendBurst( u_long bid, int npkts, int burstsize, int sourcenode, int destinationnode, double offsettime, int pcntguard, double delta, int priority, double starttime1, double starttime2, int corenode, int channel );

		// Send a control packet to Controller - added by Salek Riadi
		void sendControlPacket( Packet** pBurst, u_long bid, int npkts, int burstsize, int sourcenode, int destinationnode, double offsettime, int pcntguard, double delta, int priority );
